...
if (!empty($_POST)) {
	if (empty($_POST['usuario'])) {
		$alert = 'Ingrese el usuario identificador';
	}else{
		...
		$query = mysqli_query($conection, "SELECT * FROM administrador WHERE id= '$user'");
		...
		if ($result > 0) {
			...
			$_SESSION['idFoto'] = $data['foto'];

			header('location: admin/inicio_foto.php');
